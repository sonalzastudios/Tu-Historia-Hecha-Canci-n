...
module.exports = async function handler(req,res) {
  ...
  if (!checkBodySize(req, 5 * 1024 * 1024)) return res.status(413).json({ok:false,error:'La imagen es demasiado grande.'});
  if (!(await enforceRateLimit(req,res,'upload-cover',6,600))) return;
  try {
    ...
    if (!detected) return res.status(400).json({ok:false,error:'El archivo no parece ser una imagen JPEG, PNG o WebP válida.'});
    if (declaredMimeType && ![detected.mime, detected.mime === 'image/jpeg' ? 'image/jpg' : detected.mime].includes(declaredMimeType.toLowerCase())) return res.status(400).json({ok:false,error:'El tipo real del archivo no coincide con el formato declarado.'});
