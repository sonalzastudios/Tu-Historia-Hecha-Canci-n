...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const record={lead_id:leadId,created_at:new Date().toISOString(),status:'new',name:clean(b.name,120),business:clean(b.business,180),email,phone:clean(b.phone,80),project_type:clean(b.projectType,120),budget:clean(b.budget,120),message:clean(b.message),currency,region,detected_country:detectedCountry,region_override:Boolean(b.regionOverride||regionMismatch),region_verification_required:regionMismatch,language,privacy_consent:true,privacy_version:'2026-09-06-v1'};
